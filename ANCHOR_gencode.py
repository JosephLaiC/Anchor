...
def main():
    ...
    # Calculate the 20 closest distances to a gene for each genomic coordinate
    # step15: for each genomic coordinate, calculate the 20 closest distances to a gene; 8 hours
    # 1. The gencode file (e.g. gencode.v19.annotation.gtf) is required
    #        (downloaded from: https://www.gencodegenes.org/releases/19.html)
    # 2. The reference file with the genomic coordinates excluding blacklisted regions is needed
    # it returns a file 'top_20' - the top 20 closest distances to a gene of each coordinate under consideration
